...
public class InsertSubsistema extends Main {
	...
	private MaquinaTools mTools = new MaquinaTools();
	...
	public InsertSubsistema(String id, int xLocation, int yLocation)  {
		...
		btnInserir.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if  (isEmpty(nome)) JOptionPane.showMessageDialog(null, "Nome Inválido");
				else {
					
					int chaveMaquina = maquinas.get(nomesMaquinas[boxMaquinas.getSelectedIndex()]);
	
					try {
						mTools.inserirSubsistema(nome, chaveMaquina);
		...
		nomesMaquinas =  maquinas.keySet().toArray();
	...
	public Map<String,Integer> getMaquinas() throws SQLException{
		return mTools.getNomesMaquinas();
	}
